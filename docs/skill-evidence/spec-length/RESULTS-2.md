...
**`R3a`: no candidate beat the control — ship nothing, and record it.** That is the outcome
`R3a` step 1 names when there are no survivors.
...
**What `R4` also requires: T10 must state plainly when the null is attributable to the instrument
rather than to the candidates.** The honest answer is **partly, and the split is visible.**
...
they are.** The third revision took **62 tier-1 scorer dispatches** — 34 for a first pass that was
**discarded in full** and 32 for the pass that stands. The discard is `SCORING-3-NOTES.md` §5.1:
...
kept. Tier 2 took **18** dispatches and tier 3 **17**, plus **one** `R6` re-run.
...
Quoted from `PROTOCOL-3.md` §4 and not softened:
...
2. **One `R6` re-run, in tier 3.** `fe4059`'s first tier-3 dispatch **wrote no file** — `R6`'s first
...
3. **Tier 1's whole first pass was discarded and re-run** — 34 dispatches — under
...
4. **The tier-1 prompt carries two precedence sentences beyond the frozen text.** `PROTOCOL-3.md` §5
...
5. **Two defects in the frozen `PROTOCOL-3.md` were recorded and NOT fixed**
   ...
   move.
6. **The tier-2/tier-3 checks were re-pointed from `retention-2/` to `retention-3/`.** No frozen
   text moved — `PROTOCOL-3.md` §5 replaced the tier-1 pass those items name, so `retention-3/` *is*
   the tier-1 record — and the checks are strictly harder afterwards, recomputing 18 samples where
   3 were possible. Recorded because it is a change to what a committed check reads.
...
`PROTOCOL-3.md` §7 item 6 requires that this file **adds to** rather than replaces those lists.
Every obligation in `SCORING-2-NOTES.md` §9.5 items 1–9 and §9.5a items 10–14 stands except items 1,
4, 7(b) and §9.5a item 14, which `SCORING-3-NOTES.md` §8 item 8 itemises as discharged. In
particular **§9.5a item 12 is still open** — whether re-dispatching two transmission questions was
right — and it is now moot for the outcome, since `R7` skipped the transmission test entirely; it
remains open as a record question.

`SCORING-3-NOTES.md` is the tier-1 pass's record and §3.1, §5.1, §5.2 and §8a bound what every
tier-1 figure means. **§5.2's inter-dispatch noise floor — one to three rows per generation between
