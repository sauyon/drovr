...
**This file was revised eight times after its first commit — the first six inside window 1, the
seventh and eighth inside window 2**, and says so here rather than leaving `git log -p` as the only
witness.
...
| the commit that added this row (**T3, window 2**, the second and last — subject line begins `docs(spec-length): correct row 7's overclaim`; `git log --oneline -- docs/skill-evidence/spec-length/PROTOCOL.md` resolves it) | **Row `3096339` above claimed "no governed item touched". That was false and is corrected here, not quietly edited there.** Line 23 of this file defines the governed items as **items 1 through 14a, all sixteen, without exception** — and item 2 is one of them, so filling its size cells and adding its fourth deviation *did* touch a governed item. What window 2 permits is clarifying a governed item, never weakening one; the accurate claim is **"item 2 clarified, not weakened"**, which is what `3096339` should have said and what its commit message also got wrong. Nothing about the edit itself changes — no threshold, no rule, no gate moved, and items 3–14a stayed byte-identical — only the description of it. **`FREEZE.md`'s breach section also had a stale "the three deviations" pointer**, written before item 2 said four; corrected there in this same commit. **T7 logs this revision in `RESULTS.md` too.** |
