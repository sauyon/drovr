#!/usr/bin/env python3
"""Assemble `retention-3/<id>.json` from its `retention-3/parts/<id>-<k>.json` shards.

`PROTOCOL-2.md` item 10, which `PROTOCOL-3.md` inherits unchanged: **concatenation is assembly, not repair.** This script
...
under `R6` as `PROTOCOL-3.md` §3 narrows it, never patched. **Every shard is SHA-256
hashed as it is read and the hash is printed**, because `SCORING-2-NOTES.md` §9.6 found
one shard byte-identical to its own earlier attempt and could not tell a verbatim
re-emission from a cached completion; until that is shown otherwise an attempt is a
DISPATCH, not an independent sample.

The authority on whether an assembled verdict is well-formed is
`cli/tests/skills_valid.rs::spec_length_3_retention_verdicts_are_complete_and_quoted`,
...
Usage: assemble-retention-3.py <fixture> <id> [<id> ...]
...
def assemble(fixture, spec_id):
    ...
    for k, lo, hi in SHARDS[fixture]:
        ...
        print(f"  shard {k}: sha256 {digest}  {path.name}")
