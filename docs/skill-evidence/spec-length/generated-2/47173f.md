...
The API server is not the problem. `deployConfigService.List`
(`api-server/services/deploy_config.go:194-201`) with no name filter already returns the latest
version of every distinct config in the project. Shared configs are ordinary `deploy_config`
rows materialised into the target org + project by `reconcileShared`
(`api-server/services/deploy_config.go:484-490`, `Shared: true`), so they are already in that
listing, and the typed client method already exists:
`api.Client.ListDeployConfigs(ctx, project, qs)` (`internal/mcloudcli/api/deploy_config.go:26`).
No API or server change is in scope.

The gap is narrower than "the TUI cannot reach shared configs": it can, today, via `e` (edit) →
source = Deploy config → a 3-step `huh.Select` picker. What's missing is the direct, browsable
path on the detail screen that `V` claims to be.

## Approach

`V` opens a single browsable, collapsible tree with three levels — benchmark project → config →
version — built on the browser's existing `fieldTree` widget (`browse_fieldtree.go:151`), which
already backs the config, revision, and diff panels and supplies cursor motion, expand/collapse,
and rendering. This was chosen over a chain of modal `huh.Select` steps (the shape
`browse_source_config.go` uses for the same three-level data) because browsing sideways — what
else is in this project, what other versions does that config have — is the point of the
feature, and a stepped picker forces backing out and starting over to compare two configs. The
three levels themselves are unchanged from the stepped alternative; only the presentation is a
tree instead of sequential prompts.

Reused as-is from `browse_source_config.go`: `loadSrcConfigsCmd` → `ListDeployConfigs` +
`latestPerConfig` for listing a project's configs, `fetchDeployConfigVersions` for a config's
versions, `ListBenchmarkProjects` for the project list, the per-step `*Msg` async-guard pattern,
and the `count`-parameter pitfall (an omitted `count` is `LIMIT 0`, not unlimited —
`deploy_config.go:344-349`). Not reused: `newDeployConfigVersionPicker` and the `huh.Select` step
wiring — the tree replaces both for `V` only; they keep serving the create/edit form and
`browse_form_sync.go`.
...
- `enter` on a version leaf chooses it and arms the confirm. `enter` on a project or config
  branch toggles it — a branch is never a selection.
...
known. Each branch caches its children after the first load — re-collapsing and re-expanding
does not refetch. This is what makes the tree strictly cheaper than a flat list of every version
of every config, which would need all N version listings up front.

A flat list was rejected for a second reason: it would have to be keyed on the config row id
(`uint`), which forces a choice between dropping version selection entirely (a regression — `V`
can pick an older version today) or loading every version of every config up front. The tree
avoids both, and the `Select[uint]` collision the task originally flagged disappears because the
...
- **Linked deployment:** auto-expand its project and its config, and place the cursor on the
  current version, so the common case (bump to a newer version of the config already in use) is
...
- **Unlinked deployment:** auto-expand the sole project when the org has exactly one, otherwise
  leave all projects collapsed with the cursor on the first. This applies even to a linked
  deployment's own project — it is always auto-expanded regardless of how many projects the org
  has.

Auto-expanding on open means the opening fetch chain — projects → the linked project's configs →
the linked config's versions — is the same three requests the stepped design would have made, so
the common path costs no more than before.

### Cost of this shape

The `huh.Select` steps had `.Filtering(true)`; `fieldTree` has no type-to-filter. For an org with
many configs, browsing is by cursor alone. Adding filtering to `fieldTree` is out of scope here
(see *Follow-ups*) rather than silently dropped.

### Cross-project adoption

Every project in the org is a root of the tree, so a deployment can be switched to a config in
any of them, not just its current one, and the user can see the other projects sitting there
without committing to one first. This needs no transport change: `switchDeployConfigVersion`
(`deploy_config.go:406`) already takes `project` as a parameter, and `confirmState` already
carries `dcProject`. `completeDCSelection` populates it from the chosen leaf's project instead of
the linked config's.
...
`diff` — nothing that maps a cursor position back to a domain object. It gains one optional
field:
...
version leaf, and `enter` reads it back. Branch nodes leave it nil — that nil-ness *is* the "a
branch is never a selection" rule; no separate node-kind enum is added.

`fieldTree` also gains a `current() *fieldNode` accessor returning the node under the cursor, so
...
`dcVersionsMsg` drops the `current uint` field it had in the stepped design — the current
...
returning `overlayHandler{m.updateDCPicker, m.dcPickerView}`. A three-form `dcPickerActive()`
with a precedence order, which the stepped alternative would have needed, does not apply — there
is one form-like object, so the input-routing/render pairing invariant that file documents holds
trivially.
...
`browse_model.go:1910-1917` drops the `DeployConfigID == nil` rejection. `V` becomes valid on any
...
- Key help (`browse_model.go:200`) already reads `"switch deploy config"` — no change needed
  there, it is now simply accurate.
...
the confirm state (`dcProject`, `dcName`, `dcVersion` on `confirmState`) and already sends
`deploy_config_id` + full `configuration`. Cross-config switching needs no transport change —
`completeDCSelection` populates `dcProject`/`dcName` from the chosen config instead of the linked
one.

## Decisions on switch semantics

These change what a switch does to a running deployment, so they are decisions, not
implementation detail.

### An unlinked deployment adopting a config preserves the deployment's config as overrides
...
baseline is `nil` today, and the function returns the target verbatim — so adopting a config
...
replicas rolled onto a floating tag). The edit form already avoids this —
`completeConfigPick` (`browse_source_config.go:445`) falls back to
`overridesFromConfiguration(prior)` when there is no linked baseline — and `V` is made to match
it.

**Decision:** when the linked baseline is absent, `preserveDeploymentDivergences` stops
returning the target verbatim and instead performs a two-way deep merge: the target config is
the base, the deployment's current configuration is the overlay, and the overlay wins on
...
Implementation constraint: reusing `threeWayMerge` with an empty-map baseline is the wrong
approach, not just a shortcut — its "value only current has" branch takes `curVal` wholesale for
any key absent from the baseline (`deploy_config.go:479-481`), so an empty-map baseline would
make every top-level key the deployment has replace the target's wholesale, dropping nested keys
the target contributes. The two-way merge is implemented as a distinct, recursing map-level
helper (the map twin of `mergeJSONPtr`), not by bending `threeWayMerge`.

### Switching across config names keeps the three-way merge

The three-way merge holds fields where the deployment diverged from the config it was linked to.
Crossing to a different config, those divergences are computed against a baseline with no formal
relationship to the target. The merge is kept anyway: a pinned image surviving the switch is the
behaviour that matters most in practice, and applying the target verbatim on a name change would
reintroduce the unpinning hazard the previous decision closes. This applies identically to a
cross-project switch — the baseline is the config the deployment was linked to, wherever it
lived. The confirm copy states that overrides are carried across (see *Confirm copy*).
...
- that the deployment's overrides are carried across (the two decisions above);
...
- Cross-project adoption, via the tree's project level (a consequence of the three-level shape).
- Preserving the deployment's configuration when an unlinked deployment adopts a config — a
  change to `preserveDeploymentDivergences`, which also fixes the same gap for the existing
...
**Out of scope**
- Any API-server or schema change — the server already supports the needed listing (see
  *Problem*).
...
- A warning that editing a shared config gets reverted — moot, not deferred: the TUI and CLI have
  no deploy-config write path at all. Reads only (`internal/mcloudcli/api/deploy_config.go`); the
  sole write is `mcloud deploy-config apply` (`internal/mcloudcli/cli/deploy_config_cmd.go:86`),
  the shared-apply POST itself. There is nothing to warn about because nothing here edits a
  config — the consequence of *linking to* a shared config is covered instead by the confirm-copy
  line above.
...
2. `dcConfigsMsg` fills the right project branch with one node per config name, and two configs'
   version leaves coexist in one tree carrying distinct `pick` payloads — the regression guard
   for the `Select[uint]` collision the old design had.
...
5. `enter` on a version leaf arms `confirmState` with that leaf's project and config name (not
   the linked config's) — covering the cross-project case, where the chosen project differs from
   the linked config's.
...
11. `fieldNode.pick` is nil on every node the existing config/revision/diff tree builders produce
    (guards the shared-widget change against leaking into other panels).
...
12. Nil baseline preserves: target config + a deployment carrying a pinned `engine_spec.image`
    and no link ⇒ the pin survives, and a key only the target carries still lands. This is the
    regression guard for the unpinning hazard.
13. Nil baseline recurses: a nested key present in the target but absent from the deployment's
    config is not dropped — the guard against the wholesale-replace trap noted in the
    implementation constraint above.
14. Non-nil baseline is unchanged: existing three-way behaviour is untouched, so the
    cross-config-name case keeps carrying divergences.
...
`DeployConfigVersion`, `deployment.go:52`) — this is why the project must come from the picker or
from the linked config rather than from the deployment, and it is why the picker's project level
