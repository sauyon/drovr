...
## Two semantic decisions this exposes — both settled
...
**Decision.** When the linked baseline is absent, `preserveDeploymentDivergences` stops returning
the target verbatim and instead performs a **two-way deep merge**: the target config is the base,
the deployment's current configuration is the overlay, and the **overlay wins on conflicting
leaves** while keys only the target carries still land. That is the semantics of the form's
`mergeJSONPtr`, whose doc comment states it directly — *"deep-merge overlay onto base: new
deploy-config fields are kept, deployment-specific overrides win on conflict"*
(`browse_form_state.go:730-732`).

**Implementation constraint — do not pass an empty map into the existing merge.** Reusing
...
The two-way merge recurses. It is implemented as a distinct map-level helper (the map twin of
`mergeJSONPtr`) rather than by bending `threeWayMerge`.
...
reintroduce the exact unpinning hazard decision A closes. The confirm copy states that overrides
are being carried across (see *Confirm copy* below).
...
The `actionSwitchDeployConfig` confirm states, in one line each:
...
- **Warning that editing a shared config gets reverted** — resolved by fact, not by design:
  ...
  moves out from under you (the deployment stays pinned to the version it selected —
  versions are immutable rows). That line is part of the confirm copy above, not a blocker.
...
Context carried forward: `schemasv1.DeploymentSchema` carries **no** benchmark-project
field (only `DeployConfigID` / `DeployConfigVersion`, `deployment.go:52`), which is why the
project comes from the picker or from the linked config rather than from the deployment.
