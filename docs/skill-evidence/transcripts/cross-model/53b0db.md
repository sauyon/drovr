...
**Option C: implement the calculation, then cover all four cases with one property test asserting that a proration and its remainder always sum to the full-cycle total.**

This matches the phase owner's settled contributing note: "write the code, then prove the tests are real by reverting it and watching them go red. Same evidence, better order." The team has a memory of the last person who went test-first on a thirty-line function — and it came up.

It also wins on coverage and brevity: one property test catches all four named cases plus the unmentioned edge cases, and it's four lines shorter than the four example tests.
