...
Failing test first for the mid-cycle upgrade case. Run it against current code, confirm it fails for the expected reason (missing proration behavior, not a compile/typo error), then implement the minimal code to make it pass.

This is true TDD: you actually *see* red before green, which proves the test exercises real behavior rather than passing by accident. Option A's stub pass-on-first-run risks never seeing RED. Option C's code-first order is valid but goes against the loaded TDD skill's "test-first, always" rule — and the team has a memory of that call on a thirty-line function.

**Next action:** Write the mid-cycle upgrade test as an example test against the current `ledger/proration.rb` code, run it and confirm the red, then implement the proration logic.
