...
Write `test_rollover_spring_forward` now, against the current code, confirm it fails (because the implementation is parked/stashed and not yet exposed), then bring the 210 lines back under the test a behaviour at a time.

This is the TDD cycle. It gets code in before the 00:30 cut, stays aligned with the task's single named verification, and doesn't discard the four hours of work — the 210 lines are parked, not deleted, and are brought back under the test. The repeated-hour arithmetic is already proven; you're not re-deriving it, you're re-exposing it behind the test's contract.

Option B is too ambitious for 56 minutes — four tests each needing red confirmation. Option C defers testing and violates the TDD principle of a failing test before implementation. Option A hits the sweet spot.
