...
The 210 lines in `scratch/rollover.rb` are *proven* — you've already run them against `fixtures/dst_spring.json` and got the right 19 shifts. Option A's cost ("rebuilding 210 lines a behaviour at a time at 23:34") isn't re-typing; it's re-deriving under a clock with nothing proven at the end that isn't proven now. That's a real waste.

Option C's switch is a crutch — the code works, the output looks right, and Yusuf is on board. Deferring tests until tomorrow leaves a verified implementation untested in main for an extra cycle.

B is the right trade-off: the 210 lines go in, and the ~26 minutes left are spent on tests that matter. You've already identified what needs testing — the rollover case plus three edge cases (shift in skipped hour, weekly rule with moving anchor, fortnightly boundary). Each test should be written red against the stashed implementation before committing. That's fast, focused work with nothing to re-derive.
