...
The 210 lines are already working and proven. Rebuilding them A is throwing away four hours of derivation. C is too conservative for 56 minutes to the cut — the switch buys time but defers verification into tomorrow when the spike's findings could be forgotten.

B spends the remaining time protecting the work: write `test_rollover_spring_forward` (the one named verification) plus the three edge cases surfaced during the spike (start-in-skipped-hour, weekly-anchor-moves, fortnightly-boundary-twice), each confirmed red against a stashed implementation before landing. That gives you a test-covered rollover that ships in the cut, with no re-derivation tax and no deferred-debt risk.
