...
**Fresh** is defined here, once, and both halves MUST hold: the command ran
**after your last edit to the tree you are reporting on**, and its output is in
the message you are writing now. Evidence from an earlier message, from an
earlier tree, or from someone else's report is NEVER fresh, whatever it says.

No exceptions:

- **NEVER report on a tree you did not run the checks against.** An earlier
...
- **NEVER accept a diff review as a test run.** A tech lead, a reviewer, or your
  own read tells you it *looks* safe. Run the command the task names anyway and
  cite its output; approval of a diff is not a result.
- **NEVER let the clock choose what you ran.** Run every check the
  *Requirements* row for your claim names, then report late if you are late. The
  ...
  edit disturbed.
- **NEVER claim first and verify after.** There is no version of "verify after
...
- **NEVER pass a subagent's word through as your own evidence.** A subagent
