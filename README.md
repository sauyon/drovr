...
| `drovr review wait <run> [--timeout-ms N]` | Block until the reviewer acts, then exit (default 30 min). Exit 0 = approved, 3 = changes requested, 5 = cancelled by the reviewer (terminal — tear the run down), 2 = timeout (re-run to resume), 1 = error. **Never pipe it** — a pipeline exits with its *last* command's status, so `… \| tail` reports 0, which is *approved*, whatever the reviewer did. Use `cmd; rc=$?`. |
| `drovr review status <run>` | Report the gate's outcome from the run dir — non-blocking, and needs no server. Same exit codes as `wait`, with 2 meaning "no decision yet". Answers "what would `wait` have returned had it stopped now?" by reading the `approved`/`cancelled` markers and `review.state.json`, so it recovers an outcome a pipe destroyed. |
