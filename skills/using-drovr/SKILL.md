---
name: using-drovr
description: Use when a task is too large or too context-heavy for one session and must run as separate agent phases that hand off to each other, before picking drovr:handoff or drovr:pipeline
---
...
Drovr is a **discipline**, not an orchestration engine. It runs work as a chain of
...
the next. herdr owns the panes/sessions; drovr owns the handoff contract and run lifecycle.
...
**When NOT to use:** a task that fits comfortably in one session. Drovr's overhead (panes,
compression, gating) is not worth it for a quick edit — just do the work.

## Prerequisites (drovr errors helpfully if absent)

- **herdr** on `PATH`, plus the claude integration: `herdr integration install claude`
  (hooks Claude Code's stop event so `drovr phase wait` can detect "done"). `drovr new`
...
```
Single phase boundary (hand this finished work to one fresh agent)?  → drovr:handoff
Full brainstorm → plan → implement → review run with a spec gate?     → drovr:pipeline
```

- **drovr:handoff** — the manual primitive: run one phase, compress it, seed the next. Use
  it directly for a one-off boundary, or read it first because **pipeline is built out of it**.
- **drovr:pipeline** — the opinionated gated runner: the four-phase flow with a human
...
(prerequisites, single-writer rule, run dir at `~/.local/share/drovr/runs/<name>/`).

## The one contract that surprises everyone

`drovr phase start` spawns a **plain** `claude` — it does **NOT** inject the seed. It only
records the seed path in `state.json`. **Injecting the briefing into the fresh agent is the
skill's job**, done with `drovr phase send`. Both downstream skills encode this; do not
