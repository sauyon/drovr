...
| watch | `drovr watch [<run>] --timeout-ms <ms>` | **run backgrounded**; exits when ANY agent (of the run, or of every unarchived run) stops on a prompt drovr will not answer. `4`=needs a human, the report names the phase and quotes the prompt · `0`=nothing left to watch (every agent gone AND every run's phases finished) · `2`=timeout → re-arm · `1`=error, including "every run I was asked to watch is unreadable". Routine permission dialogs do not wake it — a running `phase wait` answers those. Safe to background BEFORE `phase start`: a run with phases outstanding keeps it watching |
