...
| watch | `drovr watch [<run>] --timeout-ms <ms>` | **run backgrounded**; exits when ANY agent (of the run, or of every unarchived run) stops on a prompt drovr will not answer. `4`=needs a human, the report names the phase and quotes the prompt · `0`=nothing left to watch · `2`=timeout → re-arm · `1`=error. Routine permission dialogs do not wake it — a running `phase wait` answers those |
